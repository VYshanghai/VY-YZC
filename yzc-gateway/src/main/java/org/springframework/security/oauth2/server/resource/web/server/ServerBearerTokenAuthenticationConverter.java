...
/**
 * A strategy for resolving <a href="https://tools.ietf.org/html/rfc6750#section-1.2"
 * target="_blank">Bearer Token</a>s from the {@link ServerWebExchange}.
 *
 * @author Rob Winch
 * @see <a href="https://tools.ietf.org/html/rfc6750#section-2" target="_blank">RFC 6750 Section 2:
	...
	public Mono<Authentication> convert(ServerWebExchange exchange) {
		return Mono.justOrEmpty(token(exchange.getRequest()))
				.map(token -> {
					...
					// TODO 添加token失效校验
					/*try {
						CheckTokenFeign checkTokenFeign = SpringContextUtil.getBeanByClass(CheckTokenFeign.class);
						checkTokenFeign.checkToken(token);
					} catch (Exception e) {
